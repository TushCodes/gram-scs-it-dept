...
def test_admin_pod_self_heals_legacy_filename_from_supabase(tmp_path, monkeypatch):
    setup_env_for_app(tmp_path)
    ...
    with app.app_context():
        ...
        db.create_all()
        c = Consignment(
            consignment_number='LEGACYADMIN1',
            status='Delivered',
            pod_image='legacy-pod.jpg',
        )
        db.session.add(c)
        db.session.commit()
        cid = c.id
        fake_supabase.store[('pod-uploads', f'{cid}/legacy-pod.jpg')] = b'legacy-admin-pod-bytes'
    ...
    get_resp = client.get(f'/admin/consignments/{cid}/pod')
    assert get_resp.status_code == 200
    assert get_resp.data == b'legacy-admin-pod-bytes'

    with app.app_context():
        row = Consignment.query.get(cid)
        assert row.pod_image == f'supabase:pod-uploads/{cid}/legacy-pod.jpg'


def test_track_pod_self_heals_legacy_filename_from_supabase(tmp_path, monkeypatch):
    setup_env_for_app(tmp_path)
    ...
    with app.app_context():
        ...
        db.create_all()
        c = Consignment(
            consignment_number='LEGACYTRK1',
            status='Delivered',
            pod_image='legacy-track-pod.jpg',
        )
        db.session.add(c)
        db.session.commit()
        cid = c.id
        fake_supabase.store[('pod-uploads', f'{cid}/legacy-track-pod.jpg')] = b'legacy-track-pod-bytes'

    get_resp = client.get('/track/pod/LEGACYTRK1')
    assert get_resp.status_code == 200
    assert get_resp.data == b'legacy-track-pod-bytes'
    assert get_resp.location is None

    with app.app_context():
        row = Consignment.query.get(cid)
        assert row.pod_image == f'supabase:pod-uploads/{cid}/legacy-track-pod.jpg'


def test_direct_pod_upload_rejects_files_over_limit(tmp_path):
    setup_env_for_app(tmp_path)
    ...
    with app.app_context():
        try:
            ...
            pass
        db.create_all()
        c = Consignment(consignment_number='BIGPOD1', status='In Transit')
        db.session.add(c)
        db.session.commit()
        cid = c.id
    ...
    resp = client.post(
        f'/admin/consignments/{cid}/pod',
        data={'file': (BytesIO(too_large), 'too-large.jpg')},
        content_type='multipart/form-data',
    )
    assert resp.status_code == 400
    body = resp.get_json()
    assert body and body.get('message') == 'POD image must be smaller than 5 MB.'
